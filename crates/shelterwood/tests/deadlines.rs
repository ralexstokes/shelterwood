...
use crate::common::{
    GatedRecorder, MessageRecorder, ReleaseGate, advance_time, assert_eventually_frozen,
    assert_quiet, poll_once, waiting::task as waiting_task,
};
use shelterwood::{
...
async fn acceptance_winning_the_deadline_withdrawal_race_succeeds() {
    ...
    gate.release();
    assert_eventually_frozen!(|| values.lock().expect("values mutex poisoned").contains(&1)).await;
    assert!(
...
async fn call_promotion_winning_the_deadline_race_reports_response_timeout() {
    ...
    advance_time(deadline).await;
    gate.release();
    assert_eventually_frozen!(|| calls.load(Ordering::SeqCst) == 1).await;
